...
public final class BrowserLayout extends RelativeLayout {
    ...
    public BrowserLayout(Context context) {
        ...
        browserWebView.setLayoutParams(params);
        ...
        int buttonWidth = DensityUtils.getDisplayMetrics(context).widthPixels / 5;
        LayoutParams buttons_params = new LayoutParams(
                buttonWidth, ViewGroup.LayoutParams.MATCH_PARENT);

        int size = DensityUtils.dip2px(context,
                HEADER_HEIGHT_DP) / 2;
    ...
    private void configButtonsContainer(LinearLayout buttonsContainer) {
        ...
        buttonsContainer.setBackgroundColor(Color.parseColor("#70000000"));
    ...
    private void configFooterView() {
        LayoutParams footer_params = new LayoutParams(
                ...
                DensityUtils.dip2px(getContext(), HEADER_HEIGHT_DP));
    ...
    private void configBackButton(Context context, LinearLayout buttonsContainer,
        ...
        if (Build.VERSION.SDK_INT < 16) {
            mBackBtn.setBackgroundDrawable(decodeImage(mBase64Drawables.getBackInactive()));
        } else {
            mBackBtn.setBackground(decodeImage(mBase64Drawables.getBackInactive()));
        }
    ...
    private void configRefreshButton(Context context, LinearLayout buttonsContainer,
        ...
        if (Build.VERSION.SDK_INT < 16) {
            mRefreshBtn.setBackgroundDrawable(decodeImage(mBase64Drawables.getRefresh()));
        } else {
            mRefreshBtn.setBackground(decodeImage(mBase64Drawables.getRefresh()));
        }
    ...
    private void configNativeButton(Context context, LinearLayout buttonsContainer,
        ...
        if (Build.VERSION.SDK_INT < 16) {
            mNativeBtn.setBackgroundDrawable(decodeImage(mBase64Drawables.getNativeBrowser()));
        } else {
            mNativeBtn.setBackground(decodeImage(mBase64Drawables.getNativeBrowser()));
        }
    ...
    private void configCloseButton(Context context, LinearLayout buttonsContainer,
        ...
        if (Build.VERSION.SDK_INT < 16) {
            mCloseBtn.setBackgroundDrawable(decodeImage(mBase64Drawables.getClose()));
        } else {
            mCloseBtn.setBackground(decodeImage(mBase64Drawables.getClose()));
        }
